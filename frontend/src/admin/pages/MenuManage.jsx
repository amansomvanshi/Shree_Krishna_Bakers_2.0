...
const MenuManagement = () => {
  ...
  useEffect(() => {
    ...
    api.get("/admin/settings/delivery-fee")
      .then((res) => {
        const settings = {
  ...
  const handleUpdateDeliverySettings = async () => {
    ...
    try {
      ...
      const { data } = await api.put("/admin/settings/delivery-fee", nextSettings);
      const savedSettings = {
  ...
  const removeChargeDraft = (index) => {
    setDeliveryDraft((current) => ({
      ...current,
      additionalCharges: current.additionalCharges.filter((_, chargeIndex) => chargeIndex !== index),
    }));
  };

  const handleUpdateSpecial = async () => {
    try {
      await api.put("/admin/settings/todays-special", { todaysSpecial: newSpecial });
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex gap-2 min-w-max p-1">
          {[
            ...
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeSection === tab.id
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center gap-3">
        <span className="text-sm font-bold text-gray-600 min-w-max">Today's Special:</span>
        {isEditingSpecial ? (
          <div className="flex items-center gap-2 w-full max-w-lg">
            ...
            />
            <button onClick={handleUpdateSpecial} className="text-green-600 text-sm font-bold bg-green-50 px-4 py-2 rounded-lg hover:bg-green-100 transition-colors">Save</button>
            <button onClick={() => { setIsEditingSpecial(false); setNewSpecial(todaysSpecial); }} className="text-gray-400 text-sm font-bold hover:text-gray-600">Cancel</button>
          </div>
        ) : (
          <div className="flex items-center gap-3 overflow-hidden">
            <span className="text-orange-600 font-bold text-md truncate">{todaysSpecial}</span>
            <button onClick={() => setIsEditingSpecial(true)} className="text-xs text-blue-500 hover:underline font-semibold flex-shrink-0">
              Edit Text
            </button>
          </div>
        )}
      ...
      {activeSection === "delivery" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-5">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-gray-800">Delivery Pricing</h2>
              <p className="text-sm text-gray-500 mt-1">
                Cart aur payment dono me yahi distance based delivery fee apply hogi.
              </p>
            </div>
            <div className="rounded-xl bg-orange-50 border border-orange-100 px-4 py-3 text-sm text-orange-800">
              Free {deliverySettings.freeDeliveryKm} km, then ₹{deliverySettings.deliveryBaseCharge} + ₹{deliverySettings.deliveryPerKmRate}/km
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <label className="block">
              <span className="text-sm font-bold text-gray-600">Free delivery up to (km)</span>
              <input
                type="number"
                min="0"
                step="0.1"
                ...
                onChange={(e) =>
                  setDeliveryDraft({ ...deliveryDraft, freeDeliveryKm: e.target.value })
                }
            ...
            <label className="block">
              <span className="text-sm font-bold text-gray-600">Charge starts from (₹)</span>
              <input
                type="number"
                min="0"
                value={deliveryDraft.deliveryBaseCharge}
                onChange={(e) =>
                  setDeliveryDraft({ ...deliveryDraft, deliveryBaseCharge: e.target.value })
                }
                className="mt-2 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-50"
              />
            </label>
            ...
            <label className="block">
              <span className="text-sm font-bold text-gray-600">Per km rate (₹)</span>
              <input
                type="number"
                min="0"
                value={deliveryDraft.deliveryPerKmRate}
                onChange={(e) =>
                  setDeliveryDraft({ ...deliveryDraft, deliveryPerKmRate: e.target.value })
                }
                className="mt-2 w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-500 focus:ring-4 focus:ring-orange-50"
              />
            </label>
          ...
          <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <div>
                <h3 className="font-bold text-gray-800">GST, Surge & Other Charges</h3>
                <p className="text-xs text-gray-500 mt-1">
                  Percentage charges item total par calculate honge. Fixed charges direct add honge.
                </p>
              </div>
              <button
                type="button"
                ...
                deliveryDraft.additionalCharges.map((charge, index) => (
                  ...
                  >
                    <input
                      type="text"
                      value={charge.name}
                      onChange={(e) => updateChargeDraft(index, { name: e.target.value })}
                    ...
                    <select
                      value={charge.type}
                      onChange={(e) => updateChargeDraft(index, { type: e.target.value })}
                      className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-500"
                    >
                    ...
                    <input
                      ...
                      min="0"
                      value={charge.value}
                      onChange={(e) => updateChargeDraft(index, { value: e.target.value })}
                      className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-500"
                    />
                    <button
                      type="button"
                      onClick={() => updateChargeDraft(index, { enabled: charge.enabled === false })}
                      className={`rounded-lg px-3 py-2 text-xs font-bold transition-colors ${charge.enabled === false
        ...
        <div className="flex gap-2 min-w-max p-1">
          {dynamicCategories.map((cat) => (
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-bold transition-all ${activeCategory === cat
                ? "bg-gray-800 text-white shadow-md"
                : "bg-white text-gray-500 hover:bg-gray-50"
                }`}
            >
          ...
          {filteredProducts.map((product) => (
            ...
            >
              ...
              <div className="flex justify-between items-start gap-2 mb-2">
                ...
                <div className="flex flex-col items-end flex-shrink-0">
                  {product.offerPrice && product.offerPrice < product.price ? (
                    <>
                      <span className="text-orange-600 font-bold text-lg">₹{product.offerPrice}</span>
              ...
              <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between gap-2">
                ...
                >
                  <div
                    className={`w-11 h-6 rounded-full relative transition-colors duration-300 ${product.isAvailable ? "bg-green-500" : "bg-red-400"
                      }`}
                  >
                    <div
                      className={`w-5 h-5 bg-white rounded-full absolute left-0.5 top-0.5 shadow-sm transition-transform duration-300 ${product.isAvailable ? "translate-x-5" : "translate-x-0"
                        }`}
                    ></div>
                  </div>
                  <span
                    className={`text-xs font-bold ${product.isAvailable ? "text-green-600" : "text-red-500"
                      }`}
                  >
                ...
                >
                  <span className={`text-xl transition-all hover:scale-125 ${product.isBestseller ? "text-yellow-400 drop-shadow-md" : "text-gray-200 grayscale hover:grayscale-0"}`}>
                    ⭐
                  </span>
                </div>
