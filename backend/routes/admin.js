...
const blogUpload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (file.fieldname === "mediaFile") {
      ...
      if (selectedMediaType === "video" && !file.mimetype.startsWith("video/")) {
        return cb(new Error("Only video files are allowed when media type is Video."));
      }

      if (selectedMediaType === "image" && !file.mimetype.startsWith("image/")) {
        return cb(new Error("Only image files are allowed when media type is Image."));
      }

      if (!["video/", "image/"].some((type) => file.mimetype.startsWith(type))) {
...
const normalizeFileUrl = (req, file) =>
  file ? `${req.protocol}://${req.get("host")}/uploads/blogs/${file.filename}` : "";
...
const handleBlogUpload = (req, res, next) => {
  blogUploadFields(req, res, (error) => {
    ...
    if (error instanceof multer.MulterError && error.code === "LIMIT_FILE_SIZE") {
      return res.status(400).json({ error: "Each blog file must be 5 MB or smaller." });
    }

    return res.status(400).json({ error: error.message || "Blog upload failed." });
  });
};
...
const buildBlogPayload = (req, existingBlog) => {
  ...
  const thumbnailUrl = req.body.thumbnailUrl?.trim() || existingBlog?.thumbnailUrl || "";
  ...
  return {
    ...
    isActive: req.body.isActive === "false" ? false : req.body.isActive !== "true" ? existingBlog?.isActive ?? true : true,
    mediaUrl: mediaFile ? normalizeFileUrl(req, mediaFile) : mediaUrl,
    thumbnailUrl: thumbnailFile ? normalizeFileUrl(req, thumbnailFile) : thumbnailUrl,
  };
};
...
const validateBlogPayload = (payload) => {
  ...
  if (!["video", "image"].includes(payload.mediaType)) return "Media type must be image or video.";
  if (Number.isNaN(payload.sortOrder)) return "Sort order must be a valid number.";
  return null;
};
...
adminRouter.post("/add_product", auth, adminCheck, validateProduct, async (req, res) => {
  try {
    const { name, price, offerPrice, category, image, description, isBestseller } = req.body;
...
adminRouter.put("/toggle-bestseller/:id", auth, adminCheck, async (req, res) => {
  try {
    const productId = req.params.id;
...
adminRouter.post("/blogs", auth, adminCheck, handleBlogUpload, async (req, res) => {
  try {
    const sourceSelectionError = validateBlogSourceSelection(req);
    if (sourceSelectionError) {
      removeLocalUpload(req.files?.mediaFile?.[0] ? normalizeFileUrl(req, req.files.mediaFile[0]) : "");
      removeLocalUpload(req.files?.thumbnailFile?.[0] ? normalizeFileUrl(req, req.files.thumbnailFile[0]) : "");
...
adminRouter.put("/blogs/:id", auth, adminCheck, handleBlogUpload, async (req, res) => {
  try {
    const existingBlog = await Blog.findById(req.params.id);
    if (!existingBlog) return res.status(404).json({ error: "Blog not found" });

    const sourceSelectionError = validateBlogSourceSelection(req, existingBlog);
    if (sourceSelectionError) {
      removeLocalUpload(req.files?.mediaFile?.[0] ? normalizeFileUrl(req, req.files.mediaFile[0]) : "");
      removeLocalUpload(req.files?.thumbnailFile?.[0] ? normalizeFileUrl(req, req.files.thumbnailFile[0]) : "");
      return res.status(400).json({ error: sourceSelectionError });
    ...
    if (validationError) {
      ...
      removeLocalUpload(req.files?.thumbnailFile?.[0] ? payload.thumbnailUrl : "");
      return res.status(400).json({ error: validationError });
    }
    ...
    if (req.files?.thumbnailFile?.[0] && oldThumbnailUrl !== blog.thumbnailUrl) {
...
adminRouter.post("/offers", auth, adminCheck, async (req, res) => {
  try {
    const offer = await Offer.create({
      ...req.body,
      code: req.body.applyType === "manual" ? String(req.body.code || "").toUpperCase() : "",
    });
...
const buildDeliverySettingsPayload = (settings) => ({
  bakeryLocation: settings.bakeryLocation ?? DEFAULT_DELIVERY_SETTINGS.bakeryLocation,
  fee: settings.deliveryFee ?? settings.deliveryBaseCharge ?? 40,
  deliveryFee: settings.deliveryFee ?? settings.deliveryBaseCharge ?? 40,
  freeDeliveryKm: settings.freeDeliveryKm ?? DEFAULT_DELIVERY_SETTINGS.freeDeliveryKm,
  deliveryBaseCharge:
    settings.deliveryBaseCharge ?? settings.deliveryFee ?? DEFAULT_DELIVERY_SETTINGS.deliveryBaseCharge,
  deliveryPerKmRate: settings.deliveryPerKmRate ?? DEFAULT_DELIVERY_SETTINGS.deliveryPerKmRate,
  additionalCharges: settings.additionalCharges ?? DEFAULT_DELIVERY_SETTINGS.additionalCharges,
});
...
adminRouter.put("/settings/delivery-fee", auth, adminCheck, async (req, res) => {
  try {
    const {
      fee,
      bakeryLocation,
    ...
    if (invalidDeliveryValue || invalidBakeryLocation) {
      return res.status(400).json({ error: "Delivery settings and bakery location must contain valid numbers" });
    }

    if (!Array.isArray(nextSettings.additionalCharges)) {
      return res.status(400).json({ error: "Additional charges must be an array" });
    }
    ...
    if (invalidCharge) {
      return res.status(400).json({ error: "Additional charge values must be valid positive numbers" });
    }
...
adminRouter.put("/settings/categories", auth, adminCheck, async (req, res) => {
  try {
    ...
    await settings.save();
    res.json({ message: "Categories updated successfully", categories: settings.categories });
  } catch (error) {
...
adminRouter.put("/settings/todays-special", auth, adminCheck, async (req, res) => {
  try {
    ...
    await settings.save();
    res.json({ message: "Today's Special updated successfully", todaysSpecial: settings.todaysSpecial });
  } catch (error) {
